...
struct sockaddr_in client_addr[8];
...
//		}
		...
		{
			id = buffer[1];
			memcpy((void *)&client_addr[id], (void *)&client, client_len);
		}
		else
		{
			id = buffer[1];
			sendto(server_sock, (char *)&buffer[2], len-2, 0, (struct sockaddr *)&client_addr[id], client_len);
		}
	}
